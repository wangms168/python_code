...
for i in range(len(usernames)):
    with create_imapObj(hostname, port, usernames[i], passwords[i], verbose=False) as (imapObj):
        ...
        (typ, [(msgXX_bytes, msgData_bytes), Rrb_bytes]) = imapObj.fetch('1',
                                                                     '(RFC822)')  # fetch()返回一个包含两个项目的tuple，第一个项目fetch()[0]是字符串'OK',是响应代码typ；
        # typ,              msg_data                     = imapObj.fetch('1', '(RFC822)')    # fetch()返回一个包含两个项目的tuple，第一个项目fetch()[0]是字符串'OK',是响应代码typ；
        ...
        print('message_ojb.keys:',
              message_ojb.keys())  # msg.keys() https://stackoverflow.com/questions/703185/using-email-headerparser-with-imaplib-fetch-in-python
        ...
        print("message_ojb['Subject']-type:",
              Subject_decode_header[0][0].decode(Subject_decode_header[0][1] or "us-ascii"))
